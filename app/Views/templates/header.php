...
<?php
// PERBAIKAN: Menggunakan closure (fungsi anonim) agar tidak terjadi redeclare error
$isActive = function(string $path): string {
    return (service('uri')->getSegment(1) === $path) ? 'active' : '';
};
?>
  ...
  <nav class="sidebar-nav">
    ...
    <div class="nav-item">
      <a href="<?= base_url('pembayaran') ?>" class="nav-link <?= $isActive('pembayaran') ?>">
        <i class="bi bi-credit-card-2-front-fill"></i> Pembayaran Penjualan
      </a>
    </div>

    <div class="nav-item">
      <a href="<?= base_url('pembayaran_pembelian') ?>" class="nav-link <?= $isActive('pembayaran_pembelian') ?>">
        <i class="bi bi-cash-stack"></i> Pembayaran Pembelian
      </a>
    </div>
